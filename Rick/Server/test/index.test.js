...
describe('Test de RUTAS', () => {
    ...
    describe('GET /rickandmorty/login', () => {
        it('La informacion de login es correcta', async () => {
            const response = (await agent.get('/rickandmorty/login?email=hornmacarena12@hotmail.com&password=asd123')).body
            expect(response.access).toEqual(true)
        });

        it('La informacion de login es incorrecta', async () => {
            const response = (await agent.get('/rickandmorty/login?email=pepito@hotmail.com&password=123')).body
            expect(response.access).toEqual(false)
        });
    });

    describe("POST /rickandmorty/fav", () => {
        const character1 = {id: '1', name: 'Yamila'}
        const character2 = {id: '2', name: 'Macarena'}
    ...
    describe("DELETE /rickandmorty/fav/:id", () => {
        ...
        it('Devuelve el arreglo correspondiente si no se elimina ningun personaje', async () => {
            const response = (await agent.delete('/rickandmorty/fav/8792')).body;
            expect(response).toContainEqual(character1);
            expect(response).toContainEqual(character2);
        });
